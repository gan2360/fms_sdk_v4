...
class ScoreOp:
    ...
    def parse_code_get_score(self, code, movements):
        ...
        if code == statusCode.TEST_PREDICTION_CONNECT_TO_START:
            ...
            change_sum = np.sum(p_13_distances > 6)  # 腰的位置移动超过6为不合格
            ...
            change_sum = np.sum(p_13_distances > 6)
            ...
            p_7 = movements[:, 7, :]
            v_7_1 = p_7 - p_1
            magnitude_7_1 = np.linalg.norm(v_7_1, axis=1)
            dot_product = np.sum(normal_vectors * v_7_1, axis=1)
            angles = np.degrees(np.arccos(dot_product / (magnitude_7_1 * magnitude_normal_vectors)))
            ...
            p_12_dis_max = max(p_12_dis_1, p_12_dis_2)  # 序列前，中，后三个时刻右前脚掌的距离变化
            ...
            # below_offset_sum = np.sum(x_dist > 20)  # 右膝盖和左脚踝距离差距过大的数量
            ...
            angles_deg_offset_sum = np.sum(angles_deg > 25)
            angles_deg_offset_rate = angles_deg_offset_sum / len  # 手臂向量与z轴单位向量大于25度的比例
            if angles_deg_offset_rate <= 0.2 and p_12_dis_max < 10:
                score = 3
            elif  angles_deg_offset_rate > 0.6 or p_12_dis_max > 0.6:
                score = 1
            else:
                score = 2
            ...
            p_6_dis_max = max(p_6_dis_1, p_6_dis_2)  # 序列中前中后，脚掌的距离
            ...
            # below_offset_sum = np.sum(x_dist > 20)
            # below_offset_rate = below_offset_sum / len  # 左膝盖和右脚踝x距离超过20的占比
            p_21 = movements[:, 21]  # 左手
            p_18 = movements[:, 18]  # 右手
            hand_vectors = p_18 - p_21  # 手臂向量
            hand_norm_vectors = np.linalg.norm(hand_vectors, axis=1)
            ...
            angles_deg_offset_sum = np.sum(angles_deg > 25)
            angles_deg_offset_rate = angles_deg_offset_sum / len  # 手臂向量与z轴单位向量夹角超过25的占比
            if angles_deg_offset_rate <= 0.2 and p_6_dis_max < 10:
                score = 3
            elif angles_deg_offset_rate > 0.6 or p_6_dis_max > 15:
                score = 1
            else:
                score = 2
            pass
            ...
            # TODO：预测直腿主动上抬-左 8
            key_frame_i = np.argmax(movements[:, 3, 2])
            ...
            magnitude_2 = np.linalg.norm(normal_vector)
            angle = 90 - np.degrees(np.arccos(dot_product / (magnitude_2 * magnitude_1)))  # 腰与两脚形成的向量与x，y面形成的夹角 , 0.5
            if angle < 15:
                score = 1
            else:
